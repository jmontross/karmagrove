...
describe BatchCharityPayment do
  ...
  before do
   @batch_complete = Batch.create!(:batch_name => "test batch that is done", :sales => 1000)
   @batch_complete_two = Batch.create!(:batch_name => "another test batch that is done", :sales => 613)
  ...
  it "should find all batches with the state 'closed'" do
    @batch_complete.close
    @batch_complete_two.close
